...
```bash
uv run python -m mychem_mcp.server --transport stdio        # default (Claude Desktop)
uv run python -m mychem_mcp.server --transport sse --host 0.0.0.0 --port 8000
uv run python -m mychem_mcp.server --transport http --host 0.0.0.0 --port 8000
```
